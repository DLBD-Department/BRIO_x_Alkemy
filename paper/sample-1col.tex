...
A set of experiments were performed to understand the behavior of the proposed approach on real data, in particular for the freqs-vs-freqs analysis. To simulate a realistic bias detection scenario, payment data from an important bank (a cash and credit card issuer) in Taiwan were used [INSERIRE REF]. The targets were credit card holders of the bank. 
...
Training and performance details of the three models are available on the GitHub repository [INSERIRE REF]. The three models have different performance in terms of accuracy and errors distribution: the idea is to verify if and how our bias detection approach can deal with different models performances. 
...
\begin{figure}[h!]
  ...
  \label{fig:exp3}
\end{figure}
Figure \ref{fig:exp3} shows the results of the third experiment. Now we are interested in trying out the other distance, the JS divergence, that is provided by the tool for the freqs-vs-freqs analysis. As expected, the distances are of smaller order of magnitude. Apart from this numerical outcome, the pattern is the same observed for Experiment 1. 

COMPLETARE CON GLI ALTRI 4 ESPERIMENTI
