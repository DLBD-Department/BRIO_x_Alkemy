...
We present a software application for the detection and analysis of biased behaviours in AI systems. The application is meant as a tool for developers and data scientists that wish to test software systems relying on probabilistic and learning mechanisms in order to detect misbehaviours related to biases and collect data about them. The ultimate goal is to provide them with useful insights and data for the improvement of the AI systems with respect to bias.


The system we are presenting is based on the theoretical, foundational works presented in \cite{dap21}, \cite{dagp22}, and \cite{gp23}, and in this paper we discuss the formal ideas behind the definition of the software (Section \ref{sec:theory}), the technical details of its implentation implementation  (Section \ref{sec:implementation}), the data obtained from a case study that validates the usefulness of the software (Section \ref{sec:validation}), and the projected further developments aimed at integrating the presented software in a more general tool for the analysis of AI systems (Section \ref{sec:conclusions}).
