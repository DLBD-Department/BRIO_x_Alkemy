...
We conclude this introduction with a brief comparison with other projects having similar purposes. Given the sensitivity and the urgency of the topic, producing an ever-growing literature on it, our exposition has no claim of being exaustive, although we try to point to a few surveys that might help systematize known approaches.

Our tool is of the so-called \quot{post-processing} kind \cite{dAlessandro2017ConscientiousCA}, meaning that the algorithm is treated as a black box and one can only observe its behaviour and intervene \textit{ex post}. Many post-processing tools define appropriate loss/cost functions $\{0,1\}^2\to\mathbb{R}$ meant to measure at each pair of labels $(\hat{y},y)$ the \quot{undesirability} of predicting $\hat{y}$ when the correct label is $y$, and then try to minimize that. This is the case, for example, of \cite{NIPS2016_9d268236,NIPS2017_b8b9c74a,6413831} and others. This approach requires that such a comparison can be made, hence has in some sense a local flavour, while we have no assumption on the \quot{correct} label and we in turn relate \emph{frequencies}, making no suppositions on single outputs but simply comparing them (either with one another, or with respect to a single, optimal one) according to what is usually called \quot{(conditional) statistical parity} or \quot{demographic parity} \cite{fuetal2020}.

The fact that our tool is not concerned with local mistakes but focuses on the presence of biases at the very abstract level of the global behaviour of a predictive model on a whole input dataset implies also that our tool does not come equipped with any native mitigation mechanism, as the outcome of our analysis does not necessarily make it clear \textit{on which points} one should focus to solve one's problem.

Though our approach might seem related, at least in its scope, to what in the literature is often called \emph{feature weighting} \cite{NINOADAN2021115424}, it is in fact very different, and possibly orthogonal in its intention: while feature weighting aims at measuring how much a single feature influences the result of an algorithm, our detection tool wishes to quantify possible imbalances in the output.
Mechanisms intimately related to feature weighting, nevertheless, will be a central component of an additional module concerning the measurement of the degree of opacity of a predictive model, which is currently under development by the authors of the present work.
...
Finally, each time we compute a divergence relative to two classes, we scale the threshold with respect to the cardinality {\color{red}  $n_D$} of the two classes. Large classes require a stricter threshold. This is a rather obvious choice related to the fact that statistical data related to a large number of individuals tend to be more precise and fine grained. We already gave an example of this few lines ago.
